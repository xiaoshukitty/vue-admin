        ...
        <div class="head">
          ...
          <div class="header">
            ...
            <div class="" ref="elSelectGuide" v-header-switch="'elSelectGuide'">
              <el-select ref="elSelect" style="width: 220px" :value="valueTitle">
                <el-option :value="valueTitle" :label="valueTitle" class="options">
                  <el-tree id="tree-option" ref="selectTree" :data="options" :props="props" :node-key="props.value"
                    :default-expanded-keys="defaultExpandedKey" @node-click="handleNodeClick" :highlight-current="true"
                    accordion>
                  </el-tree>
                </el-option>
              </el-select>
            ...
            <div class="header_hover" ref="GlobalSearch" v-header-switch="'GlobalSearch'">
              <el-tooltip class="item" effect="dark" :content="$t('headerList.GlobalSearch')" placement="bottom">
            ...
            <div class="header_hover" ref="TopicConfiguration">
              ...
              <el-button icon="el-icon-thumb" size="small" circle @click="handleTopicConfiguration"></el-button>
              <!-- </el-tooltip> -->
            </div>
            <!-- 锁屏 -->
              ...
              <el-popover placement="bottom" width="300" trigger="hover" ref="popover">
                <el-tabs v-model="activeName">
                  <el-tab-pane label="通知" name="inform">
                    ...
                    <el-button icon="el-icon-circle-close" @click="clearInfo('informsList')">清空消息</el-button>
                  </el-tab-pane>
                  <el-tab-pane label="邮件" name="email">
                    ...
                    <el-button icon="el-icon-circle-close" @click="clearInfo('emailList')">清空消息</el-button>
                  </el-tab-pane>
                </el-tabs>
            ...
            <div class="full_screen header_hover" ref="fullScreenGuide"
              ...
              <el-tooltip class="item" effect="dark" :content="fullScreenShow
                ? $t('headerList.name')
                : $t('headerList.ExitFullScreen')
                " placement="bottom">
            ...
            <el-scrollbar class="scrollbar" wrap-style="overflow-y:hidden" ref="refScrollbar">
              <div style="display: flex" ref="refScrollRefWitch">
                <div v-for="(item, index) in recordRouteList" :key="item.id" :class="[
                  'tag',
                  'list-item',
                  activeIndexRoute == item.id ? 'active' : '',
                ]" @click="routerSkip(item, index)" @contextmenu="showContextMenu($event, item, index)">
...
export default {
  ...
  created() {
    ...
    let routerGather = searchTreeCertain(
      this.$t("routerNavigation"),
      this.$route.path
    );
    this.routerPush(routerGather);
    this.isCollapse =
      localStorage.getItem("ISCOLLAPSE") == "CLOSE" ? true : false;
  },
  mounted() {
  ...
  methods: {
    ...
    tokenExpired() {
      this.$alert(
        this.$t("headerList.TokenExpired"),
        this.$t("headerList.Reminder"),
        {
    ...
    langEditClick(lang) {
      ...
      const langData = flattTree(
        JSON.parse(JSON.stringify(this.$t("routerNavigation")))
      );
    ...
    //自动滚动
    scrollToItem(index) {
      const itemElement = this.$refs.refScrollbar.$el.querySelector(
        `.list-item:nth-child(${index + 1})`
      );
      if (itemElement) {
        const scrollContainer = this.$refs.refScrollbar.$el.querySelector(
          ".el-scrollbar__wrap"
        );
        ...
        const animateScroll = (timestamp) => {
          ...
          const newScrollLeft =
            currentScrollLeft +
            (targetScrollLeft - currentScrollLeft) * easedPercentage;
    ...
    closeRoute(val) {
      ...
      if (flag == delFlag || flag == -1) {
        ...
        this.activeIndexRoute =
          this.recordRouteList[this.recordRouteList.length - 1].id;
    ...
    closeRouterBlock(command) {
      if (
        this.recordRouteList.length == 1 &&
        this.recordRouteList[0].id == "0"
      ) {
    ...
    changeTree(newValue) {
      ...
      this.$nextTick(() => {
        ...
        if (this.defaultExpandedKey.length != 0) {
          this.$refs.selectTree.store.nodesMap[
    ...
    open() {
      this.$confirm(
        this.$t("headerList.ConfirmToExitTheSystem") + "?",
        this.$t("headerList.Reminder"),
        {
          confirmButtonText: this.$t("headerList.Ok"),
...
.slide-right-leave-active {
  height: 100%;
  width: calc(100vw - var(--left-menu-width) - 40px);
